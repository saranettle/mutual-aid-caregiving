...
def view_community_visits(communityID):
    ...
    if request.method == "GET":
        ...
        query2 = ("SELECT CONCAT(Neighbors.firstName, ' ', Neighbors.lastName) AS Neighbor, "
                  "Visits.caregiver AS Caregiver, Locations.locationName AS Location, "
                  "Visits.startTime AS Time, Visits.durationHours AS Hours, VisitTypes.typeName AS Category, "
                  "Visits.visitNotes AS Notes FROM Visits "
                  ...
                  "WHERE Visits.fulfilled = 1 AND Communities.communityID = %s") % (communityID)
        cur = mysql.connection.cursor()
        cur.execute(query2)
        fulfilled_visit_data = cur.fetchall()

        # query 3 for the unfulfilled visits names
        query3 = ("SELECT Visits.visitID, CONCAT(Neighbors.firstName, ' ', Neighbors.lastName) AS Neighbor, "
                  "Locations.locationName AS Location, Visits.startTime AS Time, Visits.durationHours AS Hours, "
                  "VisitTypes.typeName AS Category, Visits.visitNotes AS Notes FROM Visits "
                  "INNER JOIN Locations ON Visits.location = Locations.locationID "
                  "INNER JOIN Neighbors ON Visits.neighbor = Neighbors.neighborID "
                  "INNER JOIN VisitTypes ON Visits.visitType = VisitTypes.visitTypeID "
                  "INNER JOIN Communities on Locations.community = Communities.communityID "
                  "WHERE Visits.fulfilled = 0 AND Communities.communityID = %s") % (communityID)
        cur = mysql.connection.cursor()
        cur.execute(query3)
        unfulfilled_visit_data = cur.fetchall()

        # query 4 for neighbor drop down
        query4 = ("SELECT Neighbors.neighborID, CONCAT(Neighbors.firstName, ' ', Neighbors.lastName) AS neighborName "
                  "FROM Neighbors "
                  "INNER JOIN CommunityHasNeighbors ON Neighbors.neighborID = CommunityHasNeighbors.neighbor "
                  "INNER JOIN Communities ON CommunityHasNeighbors.community = Communities.communityID "
                  "WHERE Communities.communityID = %s") % (communityID)
        cur = mysql.connection.cursor()
        cur.execute(query4)
        neighbor_data = cur.fetchall()

        # query 5 for location drop down
        query5 = ("SELECT Locations.locationID, Locations.locationName FROM Locations "
                  "INNER JOIN Communities ON Locations.community = Communities.communityID "
                  "WHERE Communities.communityID = %s") % (communityID)
        cur = mysql.connection.cursor()
        cur.execute(query5)
        ...
        return render_template("view_community_visits.j2", community_data=data,
                               fulfilled_visits=fulfilled_visit_data, unfulfilled_visits= unfulfilled_visit_data,
                               neighbors=neighbor_data, locations=location_data, visit_types=visit_type_data)
    ...
    if request.method == "POST":
        if request.form.get("request_visit"):
            ...
            query = ("INSERT INTO Visits (neighbor, location, startTime, durationHours, visitType, visitNotes)"
                     " VALUES (%s, %s, %s, %s, %s, %s)")
            cur = mysql.connection.cursor()
...
def answer_request(visitID):
    ...
    if request.method == "GET":
        query = ("SELECT Visits.visitID, CONCAT(Neighbors.firstName, ' ', Neighbors.lastName) AS Neighbor, "
                 "Locations.locationName AS Location, Visits.startTime AS Time, Visits.durationHours AS Hours, "
                 "VisitTypes.typeName AS Category, Visits.visitNotes AS Notes FROM Visits "
                 "INNER JOIN Locations ON Visits.location = Locations.locationID "
                 "INNER JOIN Neighbors ON Visits.neighbor = Neighbors.neighborID "
                 "INNER JOIN VisitTypes ON Visits.visitType = VisitTypes.visitTypeID "
                 "WHERE Visits.visitID =  %s") % (visitID)
        cur = mysql.connection.cursor()
        cur.execute(query)
        data = cur.fetchall()
    
        # query to pull neighbors for drop down
        query2 = ("SELECT Neighbors.neighborID, CONCAT(Neighbors.firstName, ' ', Neighbors.lastName) AS neighborName "
                  "FROM Neighbors "
                  "INNER JOIN CommunityHasNeighbors ON Neighbors.neighborID = CommunityHasNeighbors.neighbor "
                  "INNER JOIN Communities ON CommunityHasNeighbors.community = Communities.communityID "
                  "INNER JOIN Locations ON Communities.communityID = Locations.community "
                  "INNER JOIN Visits ON Locations.locationID = Visits.location WHERE Visits.visitID = %s") % (visitID)
        cur = mysql.connection.cursor()
        cur.execute(query2)
        neighbor_data = cur.fetchall()
...
def visits():
    ...
    if request.method == "GET":
        query = ("SELECT visitID, CONCAT(carereceivers.firstName,' ',carereceivers.lastName) AS \"Neighbor\", "
                 "CONCAT(caregivers.firstName,' ',caregivers.lastName) AS \"careGiverName\", startTime, durationHours, "
                 "typeName visitType, locationName location, visitNotes, fulfilled "
                 "FROM Visits "
                 ...
                 "INNER JOIN VisitTypes ON visitTypeID = visitType ORDER BY visitID")
        cur = mysql.connection.cursor()
        cur.execute(query)
        data = cur.fetchall()
...
def edit_visit(visitID):
    if request.method == "GET":
        query = ("SELECT visitID, Visits.neighbor AS neighborID, Locations.locationID AS locationID, "
                 "CONCAT(carereceivers.firstName,' ',carereceivers.lastName) AS Neighbor, "
                 "CONCAT(caregivers.firstName,' ',caregivers.lastName) AS \"careGiverName\", startTime, durationHours, "
                 "visitType, locationName location, visitNotes, fulfilled "
                 "FROM Visits "
                 "INNER JOIN Neighbors AS carereceivers ON neighbor = carereceivers.neighborID "
                 "LEFT OUTER JOIN Neighbors AS caregivers ON caregiver = caregivers.neighborID "
                 "INNER JOIN Locations ON location = locationID "
                 "INNER JOIN VisitTypes ON visitTypeID = visitType "
                 "WHERE visitID = %s") % (visitID)
        cur = mysql.connection.cursor()
        cur.execute(query)
        data = cur.fetchall()
    ...
    # query for neighbors dropdown
        ...
        return render_template("edit_visit.j2", data=data, visit_types=visit_type_data,
                               locations=location_data, neighbors=neighbor_data)
...
def edit_visit_type(visitTypeID):
    ...
    if request.method == "POST":
        ...
        if request.form.get("edit_visit_type"):
            ...
            query = ("UPDATE VisitTypes SET VisitTypes.typeName = %s, VisitTypes.typeDescription = %s "
                     "WHERE VisitTypes.visitTypeID = %s")
            cur = mysql.connection.cursor()
...
def edit_neighbor(neighborID):
    ...
    if request.method == "POST":
        ...
        if request.form.get("edit_neighbor"):
            ...
            query = ("UPDATE Neighbors SET Neighbors.firstName = %s, Neighbors.lastName = %s, "
                     "Neighbors.neighborPhone = %s WHERE Neighbors.neighborID = %s")
            cur = mysql.connection.cursor()
...
def certify_neighbors():
    ...
    if request.method == "GET":
        query = (
            "SELECT neighborHasCertificationID, CONCAT(firstName,' ',lastName) AS neighbor, "
            "certificationTitle AS certification "
...
def edit_certify_neighbors(neighborHasCertificationID):
    if request.method == "GET":
        ...
        return render_template("edit_certify_neighbors.j2", certify_neighbors=data,
                               neighbors=neighbors_data, certifications=certifications_data)
